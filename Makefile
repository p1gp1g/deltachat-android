...
new_merge: assert_rev
	...
	git tag $(tag)-chatontest$(revision)
...
download_upstream_lib:
	@if [ -z "$(tag)" ]; then echo "tag is not set" >&2; exit 1; fi;
	TAG=$(tag) ;\
	curl --follow -o /tmp/upstream.zip "https://github.com/deltachat/deltachat-android/releases/download/$${TAG}/deltachat-gplay-release-$${TAG#v}.apk"
	unzip -d /tmp/upstream /tmp/upstream.zip
	cp -r /tmp/upstream/lib libs
...
clean_test:
	...
	-git tag --delete v2.53.0-chatontest00
